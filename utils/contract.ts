...
=========================================================================
*/
export async function getContractAt<CType extends Contract>(
  abiType: string,
  address: string
) {
...
export async function deployUUPSUpgradableContract<CType extends Contract>(
  ...
  console.log(
    `${name}-implementation deployed at address: ${
      (await implementation).address
    }`
  );

  let initializeTx = await implementation.populateTransaction.initialize(
    ...initializer
  );
  ...
  if (verify === true) {
    ...
    await timeout(60000);
    console.log(
      `Verifying ${name}-implementation contract at ${
        (await implementation).address
      }...`
    );
...
export async function upgradeUUPSUpgradeableContract<CType extends Contract>(
  ...
  name = name || abiType;
  console.log(
    `Deploying Updated ${name}-implementation for proxy ${proxyAddress}`
  );

  const newImplementationFactory = await hre.ethers.getContractFactory(abiType);
  const newImplementation = await newImplementationFactory
    .connect(deployer)
    .deploy(); // Check if need constructor
  await newImplementation.deployed();

  console.log(
    `New ${name}-implementation deployed at address: ${
      (await newImplementation).address
    }`
  );

  // const proxyContract = await hre.ethers.getContractAt("ERC1967Proxy", proxyAddress);

  const oldImplementationContract = await hre.ethers.getContractAt(
    "NiftyzoneMarketplace",
    oldImplementationAddress
  );

  const upgradeTx = await oldImplementationContract.upgradeTo(
    (await newImplementation).address
  );
